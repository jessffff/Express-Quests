...
describe("PUT /api/users/:id", () => {
  ...
  describe("DELETE /api/users/:id", () => {
    it("should delete user", async () => {
      ...
      const [queryResult] = await database.query("SELECT * FROM users WHERE id=?", id);
      expect(queryResult.length).toEqual(0);
    });
  
    it("should return 404 for non-existing movie", async () => {
      const response = await request(app).delete("/api/users/9999"); // Utiliser un ID qui n'existe pas
      expect(response.status).toEqual(404);
    });
  });
});